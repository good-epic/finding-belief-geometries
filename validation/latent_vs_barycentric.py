...
def parse_args():
    ...
    parser.add_argument("--skip_vertex", action="store_true",
                        help="Skip near-vertex split; run simplex-interior split only")
    ...
    parser.add_argument("--force_k", type=int, default=None,
                        help="Override the k (simplex dimension) from cluster metadata. "
                             "Useful for comparing K=2 vs K=3 AANet fits on the same samples.")
...
def run_model_logits_for_simplex_samples(all_samples, model, hook_name, args,
                                          aanet=None, sae=None,
                                          cluster_indices_tensor=None):
    """Run model forward only on simplex samples; use saved latent_acts and bary_coords.

    When aanet/sae/cluster_indices_tensor are provided (i.e. --force_k is active),
    bary_coords are recomputed from saved latent_acts via the supplied AANet rather
    than read from the pre-saved K=3 JSONL values.

    Returns:
        latent_acts:   (N, n_latents) float32  — from saved JSONL fields
        bary_coords:   (N, k)         float32  — from saved JSONL fields or recomputed
    ...
    if aanet is not None and sae is not None and cluster_indices_tensor is not None:
        # Recompute bary_coords through the supplied AANet (used when --force_k overrides k)
        W_c = sae.W_dec[cluster_indices_tensor, :]  # (n_latents, d_model)
        lat_t = torch.tensor(latent_acts, dtype=torch.float32, device=args.device)
        X_recon = lat_t @ W_c  # (N, d_model)
        with torch.no_grad():
            _, _, emb = aanet(X_recon)
            bary_coords = aanet.euclidean_to_barycentric(emb).float().cpu().numpy()
    else:
...
def main():
    ...
    for cluster_key in cluster_keys:
        ...
        if args.force_k is not None:
            print(f"  Overriding k={k} → {args.force_k} (--force_k)")
            k = args.force_k
        ...
        # Vertex split
        # ----------------------------------------------------------------
        if vertex_samples and not args.skip_vertex:
        ...
        if args.simplex_dir:
            ...
            if simplex_records:
                ...
                lat_s, bary_s, lp_s, mask_s, vlabels_s = run_model_logits_for_simplex_samples(
                    ...
                    aanet=aanet if args.force_k is not None else None,
                    sae=sae if args.force_k is not None else None,
                    cluster_indices_tensor=cluster_indices_tensor if args.force_k is not None else None,
                )
